...
import com.example.pandica.databinding.ActivityPromenaLozinkeBinding;

public class PromenaLozinkeActivity extends AppCompatActivity {

    ActivityPromenaLozinkeBinding binding = null;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        binding = ActivityPromenaLozinkeBinding.inflate(getLayoutInflater());
    ...
    public void sacuvajLozinku(View view){

        if(MainActivity.prijavljeniKorisnik.getLozinka().equals(binding.staraLozinka.getText().toString())) {

            MainActivity.prijavljeniKorisnik.setLozinka(binding.novaLozinka.getText().toString());

            Toast.makeText(getApplicationContext(), "Lozinka je uspesno promenjena!", Toast.LENGTH_LONG).show();

            Intent intentMojProfil = new Intent(this, MojProfilActivity.class);
            startActivity(intentMojProfil);
        }
        else{
            Toast.makeText(getApplicationContext(), "Uneta lozinka nije ispravna!", Toast.LENGTH_LONG).show();
        }
    }
